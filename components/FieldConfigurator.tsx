...
const FieldConfigurator: React.FC<FieldConfiguratorProps> = ({ assets, currentEditable, onSave, onBack }) => {
  ...
  // Mapeamento amigável para os campos técnicos
  ...
  // Identificar campos presentes no banco de dados carregado
  const availableFields = useMemo(() => {
    if (assets.length === 0) return Object.keys(fieldLabels);
    
    // Pega as chaves do primeiro ativo, excluindo campos de controle interno
    const keys = Object.keys(assets[0]).filter(k => 
      !k.startsWith('_') && k !== 'id' && k !== 'TAG_INVENTARIO' && k !== 'TAG_DUPLICIDADE'
    );
    
    // Garante que campos mapeados apareçam primeiro
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 animate-fadeIn overflow-hidden">
      ...
      <div className="px-6 pt-12 pb-6 bg-slate-900 border-b border-slate-800 relative z-20 shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="bg-purple-600/20 px-3 py-1 rounded-full border border-purple-500/30 text-purple-400">
            <span className="text-[7px] font-black uppercase tracking-[0.3em]">Protocolo v24.19</span>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h2 className="text-xl font-black text-white uppercase tracking-tighter leading-none italic">Controle de Metadados</h2>
            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mt-1">Configurar Permissões de Escrita</p>
          </div>
        </div>
      </div>

      <div className="px-6 py-4 bg-indigo-950/20 border-b border-indigo-900/30 flex items-center space-x-3">
        <AlertCircle size={14} className="text-indigo-400 shrink-0" />
        <p className="text-[8px] font-bold text-indigo-200 uppercase tracking-widest leading-relaxed">
          Os campos desmarcados serão exibidos como "Apenas Leitura" para o inventariante.
        </p>
      </div>
        ...
        {availableFields.map(field => {
          ...
          return (
            <button
              ...
              className={`w-full p-5 rounded-[2rem] border transition-all flex items-center justify-between active:scale-[0.98] ${isEditable ? 'bg-indigo-900/10 border-indigo-500/40' : 'bg-slate-900 border-slate-800 opacity-60'}`}
            >
              <div className="flex items-center space-x-4 text-left">
                ...
                <div>
                  <h4 className={`text-[11px] font-black uppercase tracking-tight ${isEditable ? 'text-white' : 'text-slate-500'}`}>{label}</h4>
                  <p className="text-[7px] font-bold text-slate-600 uppercase tracking-widest mt-0.5">{field}</p>
                </div>
              </div>
              
              <div className={`w-6 h-6 rounded-full flex items-center justify-center ${isEditable ? 'text-indigo-400' : 'text-slate-800'}`}>
                {isEditable ? <CheckCircle2 size={20} /> : <div className="w-5 h-5 rounded-full border-2 border-slate-800"></div>}
              </div>
            </button>
          );
        })}
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-slate-950/90 backdrop-blur-md border-t border-slate-900 z-30 flex items-center justify-between">
         <div className="text-[8px] font-black text-slate-700 uppercase tracking-[0.4em]">Field Authority Guard</div>
         <button 
           onClick={handleSave}
           className="bg-indigo-600 text-white px-8 py-4 rounded-2xl text-[11px] font-black uppercase shadow-2xl active:scale-95 flex items-center space-x-2"
         >
            <Save size={18} />
