...
1. **`Inventariador-main/docs/ARCHITECTURE.md`** — *fonte de verdade* da arquitetura:
...
2. **`Inventariador-main/docs/COMPONENTS_MAP.md`** — mapa mestre de navegação/dependências dos componentes.
3. **`Inventariador-main/SYSTEM_INSTRUCTIONS.md`** — governança SRE do repositório (regras de roteamento e persistência).
4. **`Inventariador-main/CHANGELOG.md`** — histórico recente de versões (convenções e regressões conhecidas).
5. **`Inventariador-main/docs/HIGIENIZACAO_ENDERECO.md`** — auditoria estrutural fechada do
   campo `endereco`/assets: `public.assets` 26/26 colunas canônicas (**no-op definitivo** —
   nenhuma DDL) · `staging.assets` = schema morto, fora de escopo (Fase D).
6. **`Inventariador-main/docs/PLANO_FASE_C_HIGIENIZACAO.md`** — contrato de execução da Fase C
...
7. **`Inventariador-main/docs/SCHEMA_BASELINE.md`** — baseline congelado do
   `InventoryLocalStore` **v4** (9 tabelas + contrato de 21 colunas; travado em
...
1. `cd Inventariador-main && npx tsc -b --noEmit` → **zero erros** (estado atual
   validado limpo na main; não produzir erros novos).
2. `cd Inventariador-main && npx vitest run` → **16 arquivos / 165 testes verdes**
...
`logo.png` placeholder, `sessionStorage.clear()` do MASTER DRIVE, fluxo "base vazia"
no Gestor de Base. (Typecheck validado **limpo** na main — nenhum erro pré-existente
conhecido.)
