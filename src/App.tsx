...
const persistCrashReport = async (details: Record<string, unknown>) => {
  try {
    ...
    const report = {
      ...
      severity: 'critical',
      source: 'app',
      message: String(details.message || 'Unknown error'),
      stack: String(details.stack || ''),
      componentStack: String(details.componentStack || ''),
      url: typeof window !== 'undefined' ? window.location.href : '',
      user_email: activeUser?.email || 'anonimo@gbr.com',
      tenantId: activeUser?.tenantId || activeUser?._tenantid || sessionStorage.getItem('tenantId') || 'CICOPAL',
      payload: safeStringify(details)
    };
    await db.transaction('rw', [db.crash_reports], async () => {
      await db.crash_reports.put(report);
    });
  } catch (persistErr) {
    console.warn('Falha ao persistir crash report local:', persistErr);
  }
};
...
const disableBootLoadingFlags = () => {
  try {
    const bootWindow = window as Window & { __gbrBootLoading?: boolean; __gbrBootReady?: boolean; __gbrBootReset?: boolean };
    bootWindow.__gbrBootLoading = false;
    bootWindow.__gbrBootReady = true;
    bootWindow.__gbrBootReset = true;
    sessionStorage.setItem('gbr_boot_loading', 'false');
    sessionStorage.setItem('gbr_boot_ready', 'true');
    localStorage.setItem('gbr_boot_loading', 'false');
    localStorage.setItem('gbr_boot_ready', 'true');
  } catch {
    // noop
  }
};
...
const removerLoaderEstatico = () => {
  try {
    const loader = document.getElementById('gbr-initial-loader') || document.getElementById('app-loader') || document.getElementById('gbr-static-loader');
    if (loader) {
...
export function InitializeBootPipeline(): boolean {
  if (typeof window === 'undefined') return false;

  disableBootLoadingFlags();
  removerLoaderEstatico();

  const activeUser = sessionStorage.getItem('app_current_user');
  const hasActiveLoginFlow = !!activeUser || sessionStorage.getItem('gbr_session_active') === 'true' || sessionStorage.getItem('gbr_bootstrap_authenticated') === 'true';
  if (hasActiveLoginFlow) {
  ...
  localStorage.removeItem('gbr_kardek_history');
  localStorage.removeItem('gbr_active_tenant');
  localStorage.removeItem('gbr_admin_scope');
  localStorage.removeItem('gbr_session_mode');
  ...
  localStorage.setItem('gbr_kardek_history', JSON.stringify([AppScreen.LOGIN]));
  localStorage.setItem('app_screen_history', JSON.stringify([AppScreen.LOGIN]));
  sessionStorage.setItem('gbr_bootstrap_reset', 'true');
  return true;
}
...
const App: React.FC = () => {
  ...
  */
  ...
  useEffect(() => {
    const bootResetApplied = InitializeBootPipeline();
    if (bootResetApplied) {
      ...
      try {
        ...
        */
        ...
        if (justCleared && hasUser) {
          console.log("[SRE_NAV] Preservando viewport no Gestor de Base após purga bem-sucedida.");
  ...
  useEffect(() => {
    ...
    const initApp = async () => {
      try {
            ...
            if (savedUserStr) {
              ...
              if (parsedUser && parsedUser.email) {
                ...
                if (hasLocalUsers || assetCount > 0) {
                  ...
                  isSessionValid = true;
                  console.log(`>>> [SRE-REBOOT] Boot Resiliente (F5): Autenticação de ${parsedUser.email} e progresso físico (${assetCount} ativos) validados no IndexedDB. Preservando sessão.`);
                }
              }
            }
          }
        } catch (err) {
          console.warn(">>> [SRE-REBOOT] Falha na validação prévia de boot:", err);
        }
        ...
        if (!isNative) {
          console.log(">>> [App] Ambiente WEB / iFrame detectado. Inicialização instantânea.");
    ...
    const restoreContextFromDb = async () => {
      ...
      try {
        if (databaseMode === DatabaseMode.INTERNAL) {
          ...
          console.log(`>>> [SYSTEM_CONTEXT] Recuperado via getContextValue: Unidade=${recoveredUnit}, Campanha=${recoveredCampaign}`);
        }
      } catch (err) {
  ...
  const completeOnboarding = useCallback(() => {
    try {
      ...
      if (currentScreen === AppScreen.ONBOARDING || currentScreen === AppScreen.LOGIN) {
        console.log("Finalizando onboarding - Redirecionando usuário para MODULE_SELECTION");
